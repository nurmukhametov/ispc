...
def get_llvm_asset(llvm_version, os_name, arch):
    ...
    Raises:
        RuntimeError: If the current OS or architecture is not supported
        URLError: If there are network connectivity issues
        json.JSONDecodeError: If the GitHub API response is not valid JSON
...
def download_file(url, filename):
    ...
    Raises:
        URLError: If the URL is invalid or unreachable
        HTTPError: If the server returns an HTTP error (e.g., 404, 403)
        OSError: If there are filesystem errors (e.g., permission denied, invalid path)
...
def extract_archive(archive_path, is_windows):
    ...
    Raises:
        ImportError: On Windows if py7zr package is not installed
        tarfile.ReadError: If the tar file is corrupt or invalid
        py7zr.Bad7zFile: On Windows if the .7z file is corrupt or invalid
        OSError: If there are filesystem errors during extraction
...
def main():
    ...
    Raises:
        SystemExit:
            - If LLVM archive detection fails and ARCHIVE_URL is not set
            - If CMake configuration fails
        OSError: For filesystem operations or subprocess failures
    ...
    Examples:
        ...
        $ LLVM_HOME=/path/to/llvm python build.py
