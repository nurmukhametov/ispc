...
function(target_ll_to_cpp llFileName bit os_name filename resultFileName)
...
function(dispatch_ll_to_cpp llFileName filename resultFileName)
    set(inputFilePath builtins/${llFileName}.ll)
    set(output ${BITCODE_FOLDER}/${filename})
    add_custom_command(
        OUTPUT ${output}
...
function(builtin_to_cpp os_name target_arch filename resultFileName)
...
function(builtin_xe_to_cpp filename resultFileName)
...
function (generate_target_builtins bitcodeList dispatchList targetBuiltinsInitStr dispatchInitStr)
    ...
    foreach (ispc_target ${regular_targets})
        foreach (bit 32 64)
            foreach (os_name ${TARGET_OS_LIST_FOR_LL})
                ...
                target_ll_to_cpp(target-${ispc_target} ${bit} ${os_name} ${filename} output${os_name}${bit})
...
function (generate_common_builtins bitcodeList commonBuiltinsInitStr)
    ...
    foreach (os_name ${supported_oses})
        foreach (arch ${supported_archs})
            ...
            if ((    ${os_name} STREQUAL "web" AND NOT ${arch} STREQUAL "wasm32") OR
                (NOT ${os_name} STREQUAL "web" AND     ${arch} STREQUAL "wasm32"))
                continue()
            endif()
            if ((    ${os_name} STREQUAL "web" AND NOT ${arch} STREQUAL "wasm64") OR
                (NOT ${os_name} STREQUAL "web" AND     ${arch} STREQUAL "wasm64"))
                continue()
            endif()
            ...
            builtin_to_cpp(${os_name} ${arch} ${filename} res${bit}${os_name}${arch})
    ...
    if (XE_ENABLED AND NOT APPLE)
        ...
        builtin_xe_to_cpp(${filename} res_xe_${bit})
        list(APPEND tmpBitcodeList ${res_xe_${bit}} )
        list(APPEND BITCODE_LIB_CONSTRUCTORS "BitcodeLib(\"${filename}\", TargetOS::${target_os}, Arch::${target_arch})")
    endif()
